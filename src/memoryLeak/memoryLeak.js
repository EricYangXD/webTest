// 什么是内存泄漏
// 本质上来讲， 内存泄露是当一块内存不再被应用程序使用的时候， 由于某种原因， 这块内存没有返还给操作系统或者空闲内存池的现象。 编程语言使用不同的方式来管理内存。 这些方式可能会减少内存泄露的机会。 然而， 某一块具体的内存是否被使用实际上是一个不可判定问题（ undecidable problem）。 换句话说， 只有开发者可以搞清楚一块内存是否应该被操作系统回收。 某些编程语言提供了帮助开发者来处理这件事情的特性。 而其它的编程语言需要开发者明确知道内存的使用情况。
...
// var 来声明这个变量， 这时一个全局变量就会被创建出来。 在这个例子中， 一个简单的字符串泄露并不会造成很大的危害， 但这无疑是错误的。
// 另外一种偶然创建全局变量的方式如下：
function foo() {
...
// 像是jQuery这样的框架和库（当使用一些特定的API时候）都在废弃一个结点之前移除了listener。它们在内部就已经处理了这些事情，并且保证不会产生内存泄露，即便程序运行在那些问题很多的浏览器中，比如老版本的 IE。
