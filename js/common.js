 ...
 */
w.clickedBar=function(id,wa,ns){
	if(openw){//避免多次打开同一个页面
		return null;
	}
	if(w.plus){
		wa&&(waiting=plus.nativeUI.showWaiting("加载中...",
...
//		setTimeout(function(){//延后显示可避免低端机上动画时白屏
			openw.show(as,at);
			if (plus.webview.all().length>3) {
			for (i=1;i<plus.webview.all().length-1;i++) {
				plus.webview.all()[i].close('none',at);
			}
		}
			closeWaiting();
//		},200);
